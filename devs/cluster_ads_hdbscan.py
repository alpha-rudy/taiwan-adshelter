#!/usr/bin/env python3
"""
ADS (Air Defense Shelter) Distance-Based Clustering Tool

This script uses HDBSCAN (Hierarchical Density-Based Spatial Clustering) to generate 
clustered nodes for different zoom levels from the original NPA_Taiwan_ADShelter.osm file.
Unlike grid-based clustering, this creates clusters based on actual distance and density
of nearby shelters, resulting in more natural groupings.

Zoom Level Guidelines:
- Z17: All individual nodes (original data) - suitable for detailed view
- Z16: Distance-based clusters for medium density view
- Z15: Larger distance-based clusters for overview

HDBSCAN clustering approach:
- Z16: min_cluster_size=3, min_samples=2 (smaller, more detailed clusters)
- Z15: min_cluster_size=5, min_samples=3 (larger, overview clusters)
"""
...
try:
    ...
    print("Warning: Advanced clustering libraries not available. Install with:")
    print("pip install hdbscan scikit-learn numpy")
...
def cluster_nodes_hdbscan(nodes: List[Node], zoom_level: int) -> List[Cluster]:
    """
    Cluster nodes using HDBSCAN (Hierarchical Density-Based Spatial Clustering).
    ...
    if not CLUSTERING_AVAILABLE:
        raise ImportError("HDBSCAN and required libraries not available")
    ...
    clustering_params = {
        16: {
            'min_cluster_size': 3,
            'min_samples': 2,
            'cluster_selection_epsilon': 300,  # 300 meters
            'max_cluster_size': 50
        },
        15: {
            'min_cluster_size': 5,
            'min_samples': 3,
            'cluster_selection_epsilon': 800,  # 800 meters
    ...
    print(f"Clustering with HDBSCAN for Z{zoom_level}")
    print(f"Parameters: min_cluster_size={params['min_cluster_size']}, "
          f"min_samples={params['min_samples']}, "
          f"epsilon={params['cluster_selection_epsilon']}m")
    ...
    for label, cluster_nodes in clusters_dict.items():
        if len(cluster_nodes) < params['min_cluster_size']:
    ...
    # Handle noise points as individual clusters for completeness
    ...
    print(f"  - {len(clusters_dict)} natural clusters")
    print(f"  - {len(noise_nodes)} individual nodes (noise)")
...
def write_clustered_osm(clusters: List[Cluster], output_filepath: str, zoom_level: int):
    ...
    osm_elem.set('generator', f'ads-hdbscan-cluster-z{zoom_level}')
    
    # Add clustered nodes
    for i, cluster in enumerate(clusters):
        node_elem = ET.SubElement(osm_elem, 'node')
        node_elem.set('id', str(-3000 - i))  # Use negative IDs starting from -3000
...
def main():
    parser = argparse.ArgumentParser(description='Cluster ADS nodes using distance-based algorithms')
    ...
    if not CLUSTERING_AVAILABLE:
        ...
        print("Please install with: pip install hdbscan scikit-learn numpy")
        return
    ...
    for zoom_level in args.zoom_levels:
        ...
        # Generate clusters using HDBSCAN
        clusters = cluster_nodes_hdbscan(nodes, zoom_level)
        
        if clusters:
            # Generate output filename
            base_name = os.path.splitext(os.path.basename(args.input_file))[0]
            output_filename = f"{base_name}_HDBSCAN_Z{zoom_level}.osm"
